...
from dataloaders.dataloader_semi import Multiview_Dataset
from utils.radam import RAdam


class Discriminator(nn.Module):
    
    def __init__(self, tile_size, in_channels, **kwargs):
...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels=4, out_channels=1, feat_channels=4, num_views=4,
        ...
        # Define layer instances of the inverse generator (deconvolution and fusion network)  
    ...
    def set_kernel_weights(self, PSF):
        '''
        Initialize convolutional kernels with the given PSFs
        '''
...
class cycleGAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        self.last_views, last_PSF, self.last_gt, last_slicing = batch['view'], batch['psf'], batch['gt'], batch['slicing']
        ...
        if optimizer_idx == 0:
            ...
            self.last_SBS_sharp_pred, self.last_SBS_blurred_with_PSF = self.forward(self.last_gt, last_PSF, 'SBS')
                
            # Cycle loss
            cycle_loss_blurry = self.compute_cycle_loss(self.last_views, self.last_BSB_blurred_with_PSF)
            cycle_loss_sharp = self.compute_cycle_loss(self.last_gt, self.last_SBS_sharp_pred)
            cycle_loss = cycle_loss_blurry + cycle_loss_sharp
            ...
            total_loss = self.hparams.cycle_loss_weight * cycle_loss + LSGAN_loss
            tensorboard_logs = {'total_loss': total_loss, 'cycle_loss': cycle_loss, 'LSGAN_loss_G': LSGAN_loss}
        ...
        if optimizer_idx == 1:
            ...
            D_gt = self.discriminator_full(self.last_gt)
            validity_gt = torch.ones_like(D_gt).cuda(self.last_gt.device.index)
            LSGAN_loss_gt = self.compute_LSGAN_loss(D_gt, validity_gt)
            ...
            LSGAN_loss_sharp_pred = self.compute_LSGAN_loss(D_sharp_pred, validity_sharp_pred)

            LSGAN_loss = 0.5 * (LSGAN_loss_gt + LSGAN_loss_sharp_pred)

            tensorboard_logs = {'LSGAN_loss_full': LSGAN_loss, 'LSGAN_loss_gt_full': LSGAN_loss_gt, 'LSGAN_loss_sharp_pred_full': LSGAN_loss_sharp_pred}
        ...
        if optimizer_idx == 2:
            ...
            # Unmatched ground truth
            D_gt = self.discriminator_half(self.last_gt[:, :, self.slicing_half[0][0]:self.slicing_half[1][0],
                self.slicing_half[0][1]:self.slicing_half[1][1], self.slicing_half[0][2]:self.slicing_half[1][2]])
            validity_gt = torch.ones_like(D_gt).cuda(self.last_gt.device.index)
            LSGAN_loss_gt = self.compute_LSGAN_loss(D_gt, validity_gt)
            ...
            tensorboard_logs = {'LSGAN_loss_half': LSGAN_loss, 'LSGAN_loss_gt_half': LSGAN_loss_gt, 'LSGAN_loss_sharp_pred_half': LSGAN_loss_sharp_pred}
            
            return OrderedDict({'loss': LSGAN_loss, 'log': tensorboard_logs})            


    def validation_step(self, batch, batch_idx):    
        view, PSF, gt, slicing = batch['view'], batch['psf'], batch['gt'], batch['slicing']
        slicing_half = slicing[0]
        slicing_quarter = slicing[1]

        # Generate images
        BSB_sharp_pred, BSB_blurred_with_PSF = self.forward(view, PSF, 'BSB')
        SBS_sharp_pred, SBS_blurred_with_PSF = self.forward(gt, PSF, 'SBS')
            
        # Cycle loss
        cycle_loss_blurry = self.compute_cycle_loss(view, BSB_blurred_with_PSF)
        cycle_loss_sharp = self.compute_cycle_loss(gt, SBS_sharp_pred)
        cycle_loss = cycle_loss_blurry + cycle_loss_sharp
        ...
        return {'cycle_loss': cycle_loss, 'LSGAN_loss_G': LSGAN_loss_G, 'LSGAN_loss_full_D': LSGAN_loss_full_D, 'LSGAN_loss_half_D': LSGAN_loss_half_D} 
    ...
    def validation_end(self, outputs):
        ...
        tensorboard_logs = {'val_cycle_loss': avg_cycle_loss, 'val_LSGAN_loss_G': avg_LSGAN_loss_G, 'val_LSGAN_loss_full_D': avg_LSGAN_loss_full_D, 'val_LSGAN_loss_half_D': avg_LSGAN_loss_half_D}
    ...
    def train_dataloader(self):
         if self.hparams.train_dir is None:
            ...
            dataset = Multiview_Dataset(data_dir=self.hparams.train_dir, gt_dir=self.hparams.train_gt_dir, num_views=self.hparams.num_views, tile_size=self.hparams.tile_size, shuffle=True, data_augmentation = self.hparams.data_augmentation)
    ...
    def val_dataloader(self):
        if self.hparams.val_dir is None:
            ...
            dataset = Multiview_Dataset(data_dir=self.hparams.val_dir, gt_dir=self.hparams.val_gt_dir, num_views=self.hparams.num_views, tile_size=self.hparams.tile_size, shuffle=False, data_augmentation=False)
    ...
    def on_epoch_end(self):
        ...
        # Log SBS sharp prediction
        SBS_prediction_grid = torchvision.utils.make_grid(torch.cat([self.last_SBS_sharp_pred[:, :, slice_idx, :, :] for i in range(3)], dim=1))
        self.logger.experiment.add_image('SBS_sharp_pred', SBS_prediction_grid, self.current_epoch)

        # Log the input view at angle 0
    ...
    def add_model_specific_args(parent_parser): 
        ...
        # Data
        parser.add_argument('--train_gt_dir', default='/datasets/quad_view/train_unpaired_gt/', type=str)
        parser.add_argument('--val_gt_dir', default='/datasets/quad_view/val_unpaired_gt/', type=str)
